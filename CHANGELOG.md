...
- **OL MCP agent-surface fidelity — 4 fixes (`get_capabilities` drift, `OL_PATH_DENIED`, fail-CLOSED path sandbox, punctuation `SyntaxWarning`)**: (1) `get_capabilities` now derives its `tools` list at runtime from the live `TOOL_REGISTRY` (was a stale hardcoded list of 17; the live registry exposes 21) — the capability manifest can no longer drift from the actual tool surface. (2) Every path-denial site now reports the stable `OL_PATH_DENIED` code (9 error-envelope/task-tracker sites + 5 error envelopes + 5 warnings) — previously `OL_INVALID_INPUT`/`INVALID_PATH` with an `OL_PATH_NOT_ALLOWED:` string prefix, which clients could not switch on; `OL_PATH_DENIED` mirrors `OPP_PATH_DENIED`. (3) `get_default_validator()` is now fail-CLOSED: it raises `ValueError` when none of `MCP_ALLOWED_DIRECTORIES` / `OL_MCP_ALLOWED_DIRS` / `OL_ALLOWED_DIRECTORIES` is set (was: silently defaulted to `cwd` + `/tmp`), so an unconfigured OL MCP server fails at validator construction instead of exposing an implicit allowlist. (4) `ol_post/punctuation.py` module docstring is now a raw string, removing the invalid-escape `SyntaxWarning`.
...
- **e2e-test-suite#44 — artifact-assertion matrix (deterministic P0/P1 checks on actual produced files)**: `run_validation.py --matrix --artifacts <dir-or-zip> --module {opp,ol,orf,suite,all}` runs the assertion engine (`scripts/validation/artifact_assertions.py`) on the REAL pipeline artifacts (extracted markdown/manifests, translated docs, backfilled html/srt/json/xml/csv/docx/pptx) and writes `artifact-report.json` into the run dir, printing `ARTIFACT MATRIX: <path>`. `--artifacts` alone implies `--matrix`; any P0/P1 assertion failure drives exit 1 (with `--artifacts` the artifact matrix, not the scenario verdicts, decides the exit code). Assertion groups: the always-on hard-security group (`_no_placeholder_leak` P0, `_no_secret_leak` P0, `_no_broken_reference` P1, `_no_external_error_text` P0) plus per-module structure sets (OPP: md non-empty/html-tag-leak, manifest json parseable, csv structure; OL: target complete, no source echo, shield roundtrip; ORF: srt timecodes, html structure, json/xml parseable, docx/pptx zip integrity). Standalone CLI: `artifact_matrix.py <dir> --module <m> --out <path>` (a directory or delivery `.zip`, 01-RAW/ preferred). Four-class artifact diff: `artifact_diff.py <base> <head>` classifies new/regressed/fixed/existing-failing per artifact (missing noted separately, never a class) and exits 1 when regressed>0 or existing-failing>0 (aligned to AutoInfo #340). Reference: `docs/dev/artifact-assertion-matrix.md`.
