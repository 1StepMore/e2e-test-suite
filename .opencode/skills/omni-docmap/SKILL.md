...
**Criterion**: canonical numbers are test matrix **131 PASS / 64 SKIP / 0 FAIL**, nightly **14**, scenario library **83** = **79 tier-1 / 3 tier-2 / 1 tier-3**. Historical measurements must carry explicit markers (e.g. `(--collect-only; canonical: 14)`).

**Verify**: `python3 scripts/doc_inventory.py --check` → prints `scenarios/: 83 yaml files; tiers={'1': 79, '2': 3, '3': 1}` and verifies the canonical test matrix + nightly-test claims.
