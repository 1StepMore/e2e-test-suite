...
class TestPathMdCLI:
    def test_full_chain_cli_returns_zero(
        ...
        md_path = opp_out / f"{haier_real_docx_path.stem}.md"
        ...
        translated_md = ol_out / f"{haier_real_docx_path.stem}.md"
        assert translated_md.exists(), f"Translated MD not produced: {ol_result.stderr}"
        translated_md_content = translated_md.read_text(encoding="utf-8")
        assert original_md_content != translated_md_content, "MD content unchanged after translation"
        assert any(m in translated_md_content for m in ["[ZH]", "你好", "世界", "用户手册", "测试", "功能"]), \
            f"No translation markers in OL CLI output: {translated_md_content[:500]}"
