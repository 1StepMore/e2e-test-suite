...
| MCP | OPP/ORF MCP servers do not respond to stdio JSON-RPC even with `transport="stdio"` | **Partial fix applied**: OPP `684d6d6` and ORF `8120cbc` now call `_mcp.run(transport="stdio")` / `server.run(transport="stdio")`. **Root cause is deeper**: `fastmcp 3.4.2` in this environment reads stdin but never writes a response to the MCP client library's `initialize()` call — both the MCP client (`asyncio.wait_for(session.initialize(), timeout=10)`) and raw `subprocess` tests with proper Content-Length framing confirm the server hangs/exits without responding. OL already uses `mcp.run_stdio_async()` but also doesn't respond. **Workaround**: none in this session. **Full fix requires**: upgrading `fastmcp` to a version that correctly handles stdio, or replacing `fastmcp` with the standard `mcp` library's `Server.run(transport=StdioServerTransport())`. **Affects**: MCP transport dimension of the 4-dimension matrix. CLI happy-path covered (131/131 PASS on real corpus). | 2026-06-21 |
...
| OPP | EPUB extraction is non-deterministic (8/8 `epub→*` cells produce different MD5 between runs) | Discovered by Tier 8 equivalence check (commit `e246eda`): OPP EPUB extractor produces content in different orders between runs. Example diff: run A outputs `Hello.\n\n# Test` while run B outputs `# Test\n\nHello.`. **Root cause**: OPP EPUB extractor iterates over EPUB entries in a non-deterministic order (likely dict iteration or unsorted set). **Affects**: all 8 `epub→*` MD-path cells. **Fix**: sort EPUB entries before extraction, or use an ordered data structure. **Impact**: equivalence check fails for these 8 cells; pass/fail matrix still passes (both runs produce valid output). | 2026-06-21 |
