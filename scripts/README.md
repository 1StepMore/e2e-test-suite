...
| `setup_dev.sh` | Omni Suite dev environment setup. Detects OS + Python version, auto-clones the 3 sub-repos (OPP/OL/ORF) if missing, creates/activates shared venv, installs all modules in editable mode, copies `.env.example`, verifies pandoc. | First-time setup on a new machine or after a fresh clone. |
...
`setup_dev.sh` automatically clones the 3 sub-repos (`Omni_Pre_Processor/`, `Omni_Localizer/`, `Omni_Re_Formatter/`) if they are missing — e.g., after a fresh `git clone` of the suite. Step 1.5 runs right after OS detection and before the Python version check.

Clone URLs can be overridden via environment variables:
...
```bash
export OMNI_OPP_URL=/path/to/local/Omni_Pre_Processor
export OMNI_OL_URL=https://github.com/my-fork/Omni_Localizer.git
