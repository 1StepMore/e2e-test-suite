...
| QA system | Multi-judge LQA, calibration against human ratings, auto-gen TM, auto-gen glossary, QA regression test |
...
1. **Multi-judge** — 3 different LLM judges vote; majority wins.
   ...
   judges, not 1.
2. **Calibration against human ratings** — build a 20-doc golden
   set with human-rated quality (1-5). LQA judges must agree with
   humans on >90% of the set. This is the QA regression test.
3. **Better LLM context** — auto-gen TM and glossary give the
   ...
   judge model.
4. **QA system regression test** — runs on every commit. If the
   QA system's correlation with human ratings drops below 90%,
   the test fails.
...
### D1. Golden evaluation set
- **File**: new `Omni_Suite/eval/golden/`
- 20 documents covering: legal (zh/en), product catalog (zh),
  literature (en), technical doc (en), email (mixed), notebook
  (en), CSV (zh), XML (en)
- Each document has a human-rated reference translation
  (bilingual reviewer; disagreements resolved by 2-reviewer
  majority)
- Stored as `{doc_id}_source.md` + `{doc_id}_reference.md` +
  `{doc_id}_score.json` (per-paragraph human scores 1-5)
- **Effort**: 2 days (the bottleneck — human translation review)
...
### D3. Calibration against human ratings
- **File**: new `Omni_Localizer/src/ol_lqa/calibration.py`
- Run the multi-judge on the golden set
- Compute correlation (Spearman) between judge scores and
  human scores
- If Spearman < 0.7 on any dimension, log WARNING and emit
  metric `omni_lqa_calibration_drift{dimension=adequacy}`
- Calibration run is a CLI: `ol calibrate --golden eval/golden/`
- **Test**: `test_calibration.py` — golden set, verify
  correlation ≥ 0.7
...
- Runs the multi-judge on a 5-doc subset of the golden set
- Asserts Spearman ≥ 0.7 on all 4 dimensions
- Runs in CI nightly (not on every commit — costs ~5min LLM time)
- **Gate**: if calibration drops below threshold, the test fails
  and a human reviewer is paged

### Phase D gate
- Golden set complete (20 docs with human ratings)
- Multi-judge runs on golden set, Spearman ≥ 0.7
...
- After D3 calibration is in, set the production acceptance:
  ...
  - **Human-judge agreement ≥ 90%** (Spearman)
...
**Total: ~10 days focused work, assuming no major surprises**
...
- **Cycle**:
  ...
  2. Collect real-world LQA scores + human spot-checks
...
- `Omni_Suite/eval/golden/` directory (D1)
- `Omni_Suite/eval/golden/{doc_id}_source.md` × 20 (D1)
- `Omni_Suite/eval/golden/{doc_id}_reference.md` × 20 (D1)
- `Omni_Suite/eval/golden/{doc_id}_score.json` × 20 (D1)
...
2. **Confirm the QA system approach** — specifically D2 (multi-
   judge from 3 providers) and D3 (calibration against human-
   rated golden set). The 20-doc golden set is the biggest time
   commitment (2 days of human translation review). Acceptable?
