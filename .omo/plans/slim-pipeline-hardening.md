...
- [x] A5. OL model swap config (MiniMax-M3 → MiniMax-M2.7) for translation role — **ENABLED** (T17 PASSED: 5 units, M3=9.40, M2.7=9.90, **delta=+0.50 M2.7 WINS**, within 0.5 threshold). The 60s timeout in T-PRE-1 was a measurement artifact, not a real bug — the LLM call needs ~3.5 min/unit and the run completed successfully. New calibration report: `Omni_Localizer/scripts/reports/calibration_2026-06-07T11-52-21.731515+00-00.json`. Compared to the earlier 5-unit run (2026-06-07T05-20-53) which showed M2.7 LOST by 0.20, the new run shows M2.7 WINS by 0.50 — stronger evidence in M2.7's favor. The config loader bug (G32, separate from G31) is filed for follow-up: `omni_localizer/src/ol_config/loader.py` ignores explicit `--config` argument and uses CWD-relative lookup, but does not block the LLM call (the fallback path also resolves env-var placeholders correctly).
