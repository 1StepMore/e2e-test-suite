...
- [ ] A5. OL model swap config (MiniMax-M3 → MiniMax-M2.7) for translation role — **DORMANT** (T17 hard-gate blocked; see Q-4 in the v2 plan). T-PRE-1 was inconclusive in the current env: (a) `Omni_Localizer/src/ol_config/loader.py` ignores the explicit `--config` argument and uses CWD-relative lookup; (b) the real-LLM CLI call hangs at 60s (likely network/reachability to `api.minimaxi.com`). Per plan v2 Q-4, A5 must stay DORMANT until T17 (100-unit Chinese business-jargon calibration re-run) reports PASS. The 5-unit English calibration that exists in `Omni_Localizer/scripts/reports/calibration_2026-06-07T05-20-53.286286+00-00.json` is the prior weak evidence — it shows M3=9.80, M2.7=9.60, delta=-0.20 PASS, but on the wrong content domain (English marketing copy, not Chinese business jargon from the slim) and on the wrong sample size (5 vs the 100 the plan recommended). New gap filed: **G31** (M2.7 swap blocked in this env until T-PRE-1 root causes are fixed).
